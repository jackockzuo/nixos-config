...
## 8. 版本管理：git tag 语义化版本（2026-08-17 建立）

> 目标：任何时刻能快速回到"验证过的稳定系统"。

### 8.1 打 tag 时机（必须）

- **每次 `nixos-rebuild switch` 成功验证后**打 tag，版本号 `vX.Y.Z`：
  - `X`（主）：架构级变更（flake-parts 迁移、disko 接入、sops 迁移主版本）
  - `Y`（次）：功能级（新增性能全家桶、新模块）
  - `Z`（补丁）：bugfix / 配置修复
- tag 必须指向**已验证**的提交（`switch` 成功 + 无新增 warning）。

### 8.2 tag 流程（必须）

```bash
# 新版本验证通过后：
git tag -a v1.0.1 -m "v1.0.1 - <一句话变更摘要>"
...
```bash
git checkout v1.0.0              # 或任意已验证版本
sudo nixos-rebuild switch --flake .
git checkout main                # 确认后再回到主线
```

### 8.4 分支纪律（必须）

- `main` = 唯一长期分支，永远可构建（验证后才提交）。
- 临时分支（实验/备份）短暂存在，合并后即删；**禁止长期挂死分支**。
- 需要"稳定 vs 开发"分离时 → 打 tag 而非开 stable 分支（tag 零维护成本）。

---

## 9. 现状审计（2026-08-16 基线，迁移路线图）
